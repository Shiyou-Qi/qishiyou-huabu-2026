...
2. 如果输入已经详细,保持原意但让结构更清晰、逻辑更严密
3. 添加上下文约束和边界条件,让 AI 输出更精准
4. 只返回优化后的提示词,不要解释`,
          temperature: 0.7,
          maxTokens: 1200,
        }),
    ...
    >
      <div className="nodrag nopan relative rounded-xl border border-border/40 bg-muted/20">
        ...
        {isGenerating && reasoning && (
          <div className="m-2.5 mb-0 rounded-lg border border-indigo-200/30 bg-indigo-50/30 p-2.5">
            <button
              className="flex w-full items-center gap-1.5 text-[13px] font-medium text-indigo-500"
            ...
            {showReasoning && (
              <p className="mt-1.5 text-[14px] leading-relaxed text-indigo-700/80 whitespace-pre-wrap">{reasoning}</p>
            )}
          </div>
        )}
        ...
        {isGenerating ? (
          <div className="min-h-[96px] max-h-[400px] overflow-y-auto p-3.5 text-[13px] leading-relaxed text-foreground">
            {streamingText ? (
              <pre className="whitespace-pre-wrap font-sans">{streamingText}<span className="inline-block w-1.5 h-4 bg-primary animate-pulse align-middle ml-0.5" /></pre>
            ) : (
      ...
      <div className="-mx-3.5 mt-3 flex items-center gap-1.5 border-t border-border/40 px-3.5 pt-2.5">
        ...
        <button
          ...
          className={cn(
            ...
            'text-indigo-400 hover:text-indigo-300 hover:scale-110',
        ...
        {isGenerating ? (
          ...
          <button
            ...
            className="flex size-7 items-center justify-center rounded-full bg-primary shadow-md shadow-primary/20 transition-all hover:bg-primary/90 active:scale-95 disabled:opacity-40"
          >
